...
AICOSTCUTTERS_SERVER_PASSWORD="$PASS" bun dev serve --port 0 >/tmp/aicostcutters-serve.log 2>&1 &
echo $! >/tmp/aicostcutters-serve.pid
while ! grep -q "server listening" /tmp/aicostcutters-serve.log 2>/dev/null; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/aicostcutters-serve.log | grep -oE "[0-9]+$")
AUTH="Authorization: Basic $(printf 'aicostcutters:%s' "$PASS" | base64 | tr -d '\n')"
BASE="http://127.0.0.1:$PORT"

# Call any endpoint
curl -sS -H "$AUTH" -H "x-aicostcutters-directory: $PWD" "$BASE/global/health"

# Stop when done
kill "$(cat /tmp/aicostcutters-serve.pid)" 2>/dev/null || true
rm -f /tmp/aicostcutters-serve.pid /tmp/aicostcutters-serve.log
```

## 1. What this doc is for

Testing local backend fixes against a real running server, talking to it over HTTP the same way the VS Code extension, TUI, and `aicostcutters run --attach` do — but without any of those clients. Every request is a `curl` the agent can copy-paste.
...
## 2. `aicostcutters serve` vs `bun dev serve` — important

| Command | What it runs |
|---|---|
| `aicostcutters serve` | The npm-installed production CLI on `$PATH`. **Not the code in this repo.** |
...
Do **not** use `createAICostCuttersServer()` from `@aicostcutters/sdk/v2` to test local code: it spawns the PATH `aicostcutters` binary (`packages/sdk/js/src/v2/server.ts:38-136`), which is the wrong tool here.
...
```bash
AICOSTCUTTERS_SERVER_PASSWORD=$(openssl rand -hex 16) \
  bun dev serve --port 0 >/tmp/aicostcutters-serve.log 2>&1 &
echo $! >/tmp/aicostcutters-serve.pid
while ! grep -q "server listening" /tmp/aicostcutters-serve.log; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/aicostcutters-serve.log | grep -oE "[0-9]+$")
```
...
```bash
AICOSTCUTTERS_SERVER_PASSWORD=secret \
  bun dev serve --port 4096 --hostname 127.0.0.1 \
  >/tmp/aicostcutters-serve.log 2>&1 &
echo $! >/tmp/aicostcutters-serve.pid
while ! grep -q "server listening" /tmp/aicostcutters-serve.log; do sleep 0.1; done
PORT=4096
```
...
Omit `AICOSTCUTTERS_SERVER_PASSWORD` entirely. The server prints a warning that the server is unsecured and the auth middleware is bypassed — fine for throwaway local testing, never for anything else.
...
| Flag | Default | Notes |
|---|---|---|
| `--port` | `0` (OS-assigned) | Must be passed literally when overriding config's `server.port`. |
| `--hostname` | `127.0.0.1` | Becomes `0.0.0.0` when `--mdns` is set without an override. |
| `--mdns` | `false` | Publishes an mDNS SRV record. |
| `--mdns-domain` | `aicostcutters.local` |  |
| `--cors` | `[]` | Extra allowed origins. |

## 4. The two mandatory request knobs

### Auth header (only if `AICOSTCUTTERS_SERVER_PASSWORD` was set)

```bash
AUTH="Authorization: Basic $(printf 'aicostcutters:%s' "$AICOSTCUTTERS_SERVER_PASSWORD" | base64 | tr -d '\n')"
```

The username is literally `aicostcutters` (same as the VS Code extension). Skip this whole block if you launched without a password.

### Directory header on every call

```bash
DIR_HEADER="x-aicostcutters-directory: $PWD"
```
...
|---|---|
| `AICOSTCUTTERS_SERVER_PASSWORD` | Enable Basic auth. Omit for auth-bypassed local testing. |
| `AICOSTCUTTERS_DB=":memory:"` | Skip on-disk SQLite — hermetic runs. |
| `AICOSTCUTTERS_DISABLE_DEFAULT_PLUGINS=true` | Don't auto-load bundled plugins. |
| `AICOSTCUTTERS_WORKSPACE_ID=<id>` | Single-workspace mode; disables control-plane routes. |
| `AICOSTCUTTERS_TELEMETRY_LEVEL=off` | Disable PostHog during tests. |
| `AICOSTCUTTERS_CONFIG_CONTENT='{…}'` | Inline JSON config without writing a file. |
...
import { createAICostCuttersClient } from "@aicostcutters/sdk/v2"

const port = process.env.PORT!
const pass = process.env.AICOSTCUTTERS_SERVER_PASSWORD
const headers = pass ? { Authorization: "Basic " + Buffer.from("aicostcutters:" + pass).toString("base64") } : undefined

const client = createAICostCuttersClient({
...
```bash
PORT="$PORT" AICOSTCUTTERS_SERVER_PASSWORD="$AICOSTCUTTERS_SERVER_PASSWORD" bun /tmp/probe.ts
rm /tmp/probe.ts
```

Reminder: this script **connects to** the server you launched in Section 3 — it does not start one. `createAICostCuttersServer()` from the SDK would spawn the PATH `aicostcutters` binary (production CLI), which defeats the point of testing local code.

## 9. Pitfalls

- Running `aicostcutters serve` instead of `bun dev serve` runs the installed prod binary, not your edits.
- Missing `x-aicostcutters-directory` (or `?directory=`) returns `400` from `InstanceMiddleware`.
...
- `--port` must appear literally in `argv` to override config's `server.port` (`packages/opencode/src/cli/network.ts:45`).
- `AICOSTCUTTERS_SERVER_PASSWORD` must be set **before** launch — changing it after doesn't rotate credentials.
- When sharing `/tmp/aicostcutters-serve.log` / `.pid` across terminals, unique-suffix the paths to avoid clobbering parallel runs.
